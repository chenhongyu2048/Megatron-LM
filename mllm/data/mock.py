...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        train_size = train_val_test_num_samples[0] if train_val_test_num_samples[0] > 0 else 1000
        ...
        valid_size = train_val_test_num_samples[1] if train_val_test_num_samples[1] > 0 else 100
        ...
        # Create DataLoaders and return iterators instead of raw dataset blocks
        # using the micro_batch_size since that's what each rank operates on.
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.micro_batch_size,
            shuffle=True,
            num_workers=0,  # avoid multiprocessing issues during simple init testing
            pin_memory=True,
            drop_last=True,
        ...
        # Convert DataLoaders to Iterator here so `train.py` can call next(data_iterator)
